...
def plot(filename):
    ...
    for i in range(0, counter - 1):
        if i == 0:
            ...
            stepInterval = sumValuesZeroGSubstracted[
                           stepBoundaries[i - 1]:stepBoundaries[i]]  # zValues[stepBoundaries[i-1]:stepBoundaries[i]]
        ...
        for j in range(stepInterval.size):
            ...
            displace += velocity
        k = 0.0249
        print i, ' length: ', k * np.sqrt(abs(displace * (maximum - minimum) / (
        avg - minimum)))  # 'step number: ', i+1, ' max: ', maximum, ' min: ', minimum, ' avg: ' , avg,
...
# plot(sys.argv[1])
plot('logs/indoor_foot_mounted_sensor/30_slow_steps.csv')

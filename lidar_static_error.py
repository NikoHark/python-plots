...
def subplot_echo(echo_points,subplot_number, title, number_of_scans):
    ...
    l2 = plt.bar(np.arange(number_of_scans), layer2, color='g', bottom=tuple(echo_points[:,0]+echo_points[:,1]),edgecolor = "none")
    l3 = plt.bar(np.arange(number_of_scans), layer3, color='b', bottom= tuple(echo_points[:,0]+echo_points[:,1]+echo_points[:,2]),edgecolor = "none")
    plt.xlabel('scan number')
...
def plot_distances(filename):
    ...
    for i in range(file_line_size):
        ...
        if(timestamp_header[i] != last_timestamp):
            ...
            last_timestamp=timestamp_header[i]
    print "number of scans:", number_of_scans
    print "minimal angles per layer: \n", min_angle[0], ", ", min_angle[1], ", ", min_angle[2], ", ", min_angle[3], "\n "
    print "maximal angles per layer: \n", max_angle[0], ", ", max_angle[1], ", ", max_angle[2], ", ", max_angle[3], "\n "
    timestamps = np.zeros(number_of_scans)
    last_timestamp = timestamp_header[0]
    timestamps[0] = timestamp_header[0]
    counter = 0
    ...
    fig = plt.figure(figsize=(16.0, 16.0))
    plt.title(filename)
    subplot_distances(np.trim_zeros(means[0]), np.trim_zeros(mins[0]), np.trim_zeros(maxs[0]), np.trim_zeros(angles[0]), "layer 0", 1, color='r')
    subplot_distances(np.trim_zeros(means[1]), np.trim_zeros(mins[1]), np.trim_zeros(maxs[1]), np.trim_zeros(angles[1]), "layer 1", 2, color='y')
    subplot_distances(np.trim_zeros(means[2]), np.trim_zeros(mins[2]), np.trim_zeros(maxs[2]), np.trim_zeros(angles[2]), "layer 2", 3, color='g')
    subplot_distances(np.trim_zeros(means[3]), np.trim_zeros(mins[3]), np.trim_zeros(maxs[3]), np.trim_zeros(angles[3]), "layer 3", 4, color='b')
    plt.show()
...
def subplot_distances(distance_means, distance_mins, distance_maxs, angles, title, layer_number, color):
    ...
    plt.errorbar(angles, distance_means, yerr = [distance_means-distance_mins, distance_maxs-distance_means], fmt='x', color=color)
...
plot_distances("/home/niko/dev/workspaces/python/plots/logs/lidar_raw_wall/mid_const.csv")